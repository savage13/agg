...
main() {
    ...
    double h = value(font + 4);
    uint16_t indices = value(font);
    uint16_t glyphs = indices + 257*2;
    printf("unsigned char size: %lu %lu %d\n", sizeof(uint8_t), sizeof(font), 257*2);
    printf("height: %f\n", h);
    printf("indices: %d\n", indices);
    printf("glyphs: %d\n", glyphs);
    char c = '!';
    unsigned int idx = c & 0xff;
    printf("index: %d\n", idx);
    idx <<= 1; /* Multiply by two */
    printf("index: %d\n", idx);
    unsigned int b = glyphs + value(font + indices + idx);
    unsigned int e = glyphs + value(font + indices + idx + 2);
    printf("b,e %d %d\n", b, e);
    for(int i = b; i < e; i+=2) {
        //printf("dx %d \n", dx);
        //printf("0x%02x 0x%02x\n", font[i],font[i+1]);
        int dx = (signed int)(int8_t)(font[i]);
        int8_t yc = font[i+1];
        int8_t yf = font[i+1] & 0x80;
        //printf("yc yf %d %d\n", yc, yf);
        yc <<= 1;
        //printf("yc yf %d %d\n", yc, yf);
        yc >>= 1;
        //printf("yc yf %d %d\n", yc, yf);
        int dy = (int)yc;
        printf("dx, dy: %3d %3d (%s) [0x%02x 0x%02x]\n", dx, dy, (yf)?"move":"line", font[i], font[i+1]);
    }
    return 0;
}
